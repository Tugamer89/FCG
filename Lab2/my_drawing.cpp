...
struct Shape {
    ...
    Shape() = default;
    Shape(size_t type, size_t color, sf::Vector2f position)
...
struct State {
    const std::vector<sf::Color> colors = {sf::Color::White,  sf::Color::Red,  sf::Color::Green,
                                           sf::Color::Blue,   sf::Color::Cyan, sf::Color::Yellow,
                                           sf::Color::Magenta};
    ...
    sf::Vector2f enforceTopLeftBounds(const sf::Vector2f& pos, float radius) const {
        return {std::max(pos.x, radius), std::max(pos.y, menu_height + radius)};
    }
};
...
void handle(const sf::Event::KeyPressed& keyPressed, State& gs) {
    ...
    else if (gs.selected_shape != -1 &&
             (keyPressed.code == Delete || keyPressed.code == Backspace)) {
...
void handle(const sf::Event::MouseMoved& mouseMoved, State& gs) {
    ...
    selected_shape.position =
        gs.enforceTopLeftBounds(selected_shape.position + offset, selected_shape.size / 2.f);
}
...
void handle(const sf::Event::MouseButtonPressed& mouseBP, State& gs) {
    ...
    if (menuArea.contains(mouse_pos)) {  // Menu
...
void handle(const sf::Event::MouseButtonReleased& mouseBP, State& gs) {
    if (mouseBP.button == sf::Mouse::Button::Right) gs.selected_shape = -1;
}
...
void doGUI(State& gs) {
    ...
    for (size_t i = 0; i < gs.colors.size(); ++i) {
        ...
        button.setPosition(
            {static_cast<float>(gs.window.getSize().x) - button_size * (static_cast<float>(i) + 1),
             0.f});
        gs.window.draw(button);
    }
}
